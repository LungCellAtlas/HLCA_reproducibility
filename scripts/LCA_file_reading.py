...
def read_file_Meyer_2021(project_dir, verbose=True):
    # read in file names and files
    file_paths_long = glob.glob(f"{project_dir}04_Counts/*/matrix.mtx.gz")
    sample_names_and_paths_unfiltered = dict()
    for path in file_paths_long:
        sample_names_and_paths_unfiltered[path.split("/")[-2]] = path
    # we need to kick out a few samples for now, since we have no annotations for
    # them yet.
    # NOTE! one of these files is an aggregation of two samples (since they came
    # from the same library, but different sequencing machines, and were
    # therefore pooled in a cellranger run), but one of those samples
    # is now included in the annotated samples. Once we decide to include
    # this (now unannotated) aggregated sample we need to remove the single
    # sample "5841STDY7991475" from the dataset!
    # WAIT FOR ELO'S CONFIRMATION BEFORE MAKING THIS FINAL!
    unannotated_samples = [
        "WSSS_A_LNG8708934",
        "WSSS_A_LNG8813290",
        "WSSS_A_LNG8986832",
        ...
        "WSSS_A_LNG8986836",
        "WSSS_A_LNG8986837",
        "5841STDY7991475"  # This sample IS in the cell metadata file, but
        # actually corresponds to the aggragated sample of our count matrices:
        # "aggr_30328_5841STDY7991475_and_32400_WSSS_A_LNG8708934"
        # (based on correspondence with Elo Madissoon)
        # so we'll import the aggregated sample instead.
    ]
    # filter
    sample_names_and_paths = {
        sample: path
        for sample, path in sample_names_and_paths_unfiltered.items()
        if sample not in unannotated_samples
    }
    adatas = dict()
    for sample_name, file_path_long in sample_names_and_paths.items():
        adata = sc.read_10x_mtx(
            "/".join(file_path_long.split("/")[:-1]), var_names="gene_ids"
        )
        # remove "feature_types" column (Gene Expression for all)
    ...
    # read in metadata (cell-level)
    meta_cell_unf = pd.read_csv(
        project_dir + "01_Metadata/lung_5loc_sc_sn_h5ad_obs.csv", index_col=0
    )
    # remove -[number] suffixes from index, so that indices
    # correspond to adata indices
    meta_cell_unf.index = [
        "-".join([string for string in bc.split("-")[:-1] if string != "1"])
        for bc in meta_cell_unf.index
    ]
    # keep only cells from our samples of interest:
    # note that we add 5841STDY7991475 here, since this is the name for the
    # aggregated sample IN THE METADATA (but not in our count matrices/adata)
    samples_to_keep = list(sample_names_and_paths.keys()) + ["5841STDY7991475"]
    meta_cell = meta_cell_unf.loc[meta_cell_unf.Sample.isin(samples_to_keep), :].copy()
    # correct sample and barcode names from "5841STDY7991475" sample,
    # which is actually the aggregated sample:
    cells_from_renamed_sample_idc = meta_cell.Sample == "5841STDY7991475"
    cells_from_renamed_sample = meta_cell.loc[cells_from_renamed_sample_idc, :].index
    cells_from_renamed_sample_corrected = [
        "-".join(
            [bc.split("-")[0], "aggr_30328_5841STDY7991475_and_32400_WSSS_A_LNG8708934"]
        )
        for bc in cells_from_renamed_sample
    ]
    meta_cell.rename(
        index=dict(zip(cells_from_renamed_sample, cells_from_renamed_sample_corrected)),
        inplace=True,
    )
    if verbose:
        print("cells from samples found in metadata:", meta_cell.shape[0])
    # subset adata:
    cells_to_keep = adata.obs.index.isin(meta_cell.index)
    if verbose:
        print(
            "of those, n cells found in anndata (count matrices):", sum(cells_to_keep)
        )
        print(
    ...
    adata.obs["original_celltype_ann"] = meta_cell.loc[adata.obs.index, "Celltypes_int"]
    # add further annotations:
    adata.obs["study_long"] = "Sanger_Meyer_2021_unpubl"
    adata.obs["study"] = "Meyer_2021"
    adata.obs["last_author_PI"] = "Meyer"
    return adata
